...
export function EducationSection() {
  return (
    <motion.section
      ...
      className="py-12"
    >
      <h2 className="text-2xl font-bold mb-8 text-[var(--foreground)]">Education</h2>
          ...
          >
            ...
            <div className="flex-1">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-2">
                <strong className="text-[var(--foreground)]">{item.degree}</strong>
                <span className="text-sm text-[var(--muted-foreground)]">{item.date}</span>
              </div>
              <p className="text-sm text-[var(--accent-light)] font-medium">{item.school}</p>
              {item.cgpa && (
                <p className="text-sm text-[var(--muted-foreground)] mt-1">
                  CGPA: <span className="font-semibold text-[var(--foreground)]">{item.cgpa}</span>
                </p>
              )}
            </div>
          </motion.div>
